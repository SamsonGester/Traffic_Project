...
int main() {
    ...
    */
    ...
    processMulti(pq, scenarios, num_scenarios);
...
void printTraffic(PriorityQueue pq, char* start_sign, char* start_lane, char* end_sign, char* end_lane) {
    ...
    for (int i = 0; i < pq.count; i++) {
        fprintf(pf, "Priority %d: %-10s %-10s %d seconds\n", i, pq.arr[i].priority, pq.arr[i].activity.sign, pq.arr[i].activity.lane, pq.arr[i].seconds);
    }
...
void processMulti(PriorityQueue pq, char scenarios[][2][15], int num_scenarios) {
    FILE *pf = fopen("traffic_result.dat", "w");
    if (pf == NULL) {
    ...
    for (int i = 0; i < num_scenarios; i++) {
        ...
        fprintf(pf, "Scenario %d: Time from %s %s to pedestrian main = %d seconds\n",
